...
#    ∇w .= 0.0
    ...
    print("SIZE TIME\n")
    print(zs[length(zs)-1])
    print(sigmoid_prime.(zs[length(zs)-1]))
    print(activations[length(activations)-1])
    print(size(y))
    print(cost((activations[length(activations)-1]),y))
    #δ = cost((@view activations[length(activations)-1]),y) * 
